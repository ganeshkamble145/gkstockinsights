...
import { ScoringLegend } from "@/components/report/ScoringLegend";
...
export function ScreenerDashboard({ kind, onBack }: { kind: Kind; onBack: () => void }) {
  ...
  const fetchPicks = async () => {
    setError(null);
    try {
      const res = await run({ data: { kind, apiKey: apiKey || undefined } });
      if (res.error) {
        setError(res.error);
  ...
  const title =
    kind === "penny"
      ? "Penny Stocks — Top 10 Ranked Picks"
      : "NIFTY 100 — Top 10 Ranked Picks";
  const subtitle =
    kind === "penny"
  ...
  const ranked: RankedRow[] = useMemo(() => {
    ...
    return result.picks
      .map((stock) => {
        ...
        const roe = parseNumeric(stock.roe);
        const roce = parseNumeric(stock.roce);
        const de = parseNumeric(stock.de);
        const pledge = parseNumeric(stock.promoterPledging);
        
        // --- Fix: Derived Upside (Programmatic over AI Hallucination) ---
        const targetNum = parseNumeric(stock.analystTarget);
        let upsidePct: number | null = parseNumeric(stock.upsidePct);
        ...
        return { stock, symbol, score, liveState, derivedUpside: upsidePct };
      })
  ...
  useEffect(() => {
    if (result && result.picks.length > 0) {
      // Use the first ticker + count as a stable ID for this session
      const resultId = `${kind}_${result.picks[0]?.ticker}_${result.picks.length}`;
      if (lastLoggedResultId.current === resultId) return;
      lastLoggedResultId.current = resultId;

      console.log("DEBUG: Logging triggered for", kind);
      toast.error(`DEBUG: Logging started for ${kind}`);
      toast.info(`AI performance tracking active for ${kind}...`);
      
      result.picks.slice(0, 10).forEach((stock) => {
        const symbol = tickerToSymbol(stock.ticker);
        const live = quoteOf(liveQuotes[symbol]);
        
        logRecommendation({
          symbol: symbol,
          tab_type: kind,
          recommendation: recommendationFor(0).label, 
          strategy: stock.thesis,
          entry_price: live?.price ?? 0, 
          target_price: parseNumeric(stock.analystTarget) ?? ( (live?.price ?? 100) * 1.25),
          stop_loss: (live?.price ?? 100) * 0.90,
          composite_score: 0,
  ...
  }, [result, kind]); // Removed fromCache dependency to log on load if result is present
  ...
  const filtered = useMemo(() => {
    return activeSector === "All" ? ranked : ranked.filter((r) => r.stock.sector === activeSector);
  }, [ranked, activeSector]);
      ...
      {!loading && !error && result && (
        <>
          ...
          <p className="text-[11px] text-muted-foreground border-t border-border pt-4 mt-4 leading-relaxed">
            Sources: {result.sources?.join(" · ") ?? "Screener.in"}. SEBI disclaimer: this is a fundamental screening
...
function StockCard({ row, rank, kind, budget, onRetry }: { row: RankedRow; rank: number; kind: Kind; budget: number; onRetry?: () => void }) {
  ...
  const { stock, score, liveState, symbol, derivedUpside } = row;
  const live = quoteOf(liveState);
  ...
  return (
    <div className="rounded-xl border border-border bg-card p-5">
      ...
      <div className="mt-3 grid grid-cols-4 gap-1.5 text-[10px]">
        <ScoreBit label="Qual" v={score.quality} />
        <ScoreBit label="Debt" v={score.leverage} />
        <ScoreBit label="Val" v={score.valuation} />
        ...
        <div className="mt-3 space-y-3 border-t border-border pt-3">
          ...
          {stock.analystTarget && (
            <div className="text-xs">
              ...
              {derivedUpside !== undefined && (
                <span className={cn(
                  "font-medium",
                  derivedUpside > 0 ? "text-emerald-600 dark:text-emerald-400" : "text-red-600 dark:text-red-400"
                )}>
                  {" "}({derivedUpside > 0 ? "+" : ""}{derivedUpside.toFixed(1)}%)
                </span>
              )}
            </div>
          )}
...
function LoadingState() {
  return (
    <div className="space-y-4">
      <p className="text-sm text-muted-foreground animate-pulse">Researching 10 picks… this can take 15–30 seconds.</p>
