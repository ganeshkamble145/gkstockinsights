...
export function FnoDashboard({ onBack }: { onBack: () => void }) {
  ...
  useEffect(() => {
    if (result && ranked.length > 0) {
      const resultId = `fo_${result.picks[0]?.symbol}_${result.picks.length}`;
      if (lastLoggedResultId.current === resultId) return;

      const hasLive = ranked.filter(r => quoteOf(r.liveState)).length >= 3;
      if (!hasLive) return;

      lastLoggedResultId.current = resultId;
      toast.info("AI performance tracking active for F&O...");
      ...
      ranked.slice(0, 10).forEach((r) => {
        ...
        logRecommendation({
          ...
          strategy: `${r.stock.trend} Strategy: ${r.stock.iv}`,
          entry_price: live.price,
          target_price: live.price * 1.05, 
          stop_loss: live.price * 0.98,    
          composite_score: r.score.total,
  ...
  const filtered = useMemo(() => {
    ...
    if (budget === Infinity) return bySector;
    
    return bySector.filter(r => {
      // Try to extract number from maxRisk (e.g. "₹5,000" -> 5000)
      const riskStr = r.stock.strategy.maxRisk.replace(/[^0-9]/g, "");
      const riskNum = parseInt(riskStr, 10);
      return isNaN(riskNum) || riskNum <= budget;
    });
      ...
      <header className="mb-4 flex items-start justify-between gap-3 flex-wrap">
        <div>
          <h1 className="text-xl sm:text-2xl font-semibold tracking-tight">
            F&amp;O Trading — Top 10 Ranked Picks
          </h1>
      ...
      {loading && (
        <div className="space-y-2">
          <p className="text-sm text-muted-foreground">
            Scanning F&amp;O universe and ranking 10 picks… 15–30 seconds.
          </p>
        ...
        <>
          ...
          <div className="flex flex-wrap items-center justify-between gap-2 mb-3">
            ...
            <div className="flex items-center gap-2">
              <select
                value={budget === Infinity ? "" : budget}
                onChange={(e) => setBudget(e.target.value === "" ? Infinity : Number(e.target.value))}
                className="bg-card border border-border rounded-full px-3 py-1 text-xs text-muted-foreground hover:border-foreground/40 hover:text-foreground focus:outline-none transition-colors cursor-pointer"
              >
          ...
          <p className="text-[11px] text-muted-foreground border-t border-border pt-4 mt-6 leading-relaxed">
            Sources: {result.sources?.join(" · ") ?? "NSE/BSE/Screener.in"}. SEBI disclaimer: educational F&amp;O screening
